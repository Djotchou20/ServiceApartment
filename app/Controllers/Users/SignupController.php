...
class SignupController extends BaseController
{
    ...
    {
        $page = new \stdClass();
        $page->title = 'Service Apartment ~ Register As A User';
        ...
        return view('users/register_users', $data);
    }
    ...
    public function register_users()
    {
        ...
        $affected = $UsersModel->insert($formData);

        if ($affected > 0) {
            $this->sendRegistrationEmail($formData['email'], $formData['name']);
            return redirect()->to('user-login')->with('success', 'Registration successful. Please check your email for confirmation.');
        } else {
